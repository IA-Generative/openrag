...
- Le back-channel logout est configuré sur **une seule URL** (`https://api.openrag-mirai.fake-domain.name/auth/backchannel-logout`) — limitation Keycloak qui n'accepte qu'un seul URL valide dans `attributes.backchannel.logout.url` (les espaces invalident le champ à l'import). Conséquence : seule la VM 2 (déploiement actuel) reçoit la notification de révocation IdP. La VM 1 (legacy) et les futures bascules sur `numerique-interieur.com` n'ont pas de back-channel logout actif ; leurs sessions expirent naturellement à l'échéance de l'access token (30 min par défaut). Si plus tard vous voulez du back-channel cross-VM, deux options : (a) un reverse proxy interne qui fan-out la notification, (b) RP-initiated logout depuis chaque UI (mécanisme alternatif via redirect).
