...
async def test_partition_chat_retrieves_factual_statement_regardless_of_classifier_or_metadata(
    ...
    if operation == "chat_stream":
        ...
        # On this release line `extra` still travels as a JSON string, not an object.
        extra = json.loads(next(event["extra"] for event in reversed(events) if event.get("extra")))
    else:
